jQuery(window).on('elementor/frontend/init', () => {
    ...
    const GridOspitiHandler = function ($scope, $) {
        ...
        function updateContent(card) {
            ...
            detailsBox.querySelector('.gd-name').textContent = dataNode.querySelector('.data-name').textContent;
            detailsBox.querySelector('.gd-job').textContent = dataNode.querySelector('.data-job').textContent;
            detailsBox.querySelector('.gd-info').innerHTML = dataNode.querySelector('.data-info').innerHTML;
            detailsBox.querySelector('.gd-bio').innerHTML = dataNode.querySelector('.data-bio').innerHTML;
        }
        ...
        cards.forEach((card, index) => {
            card.addEventListener('click', () => {
                ...
                const isSameRow = detailsBox.previousElementSibling === lastInRow || 
                                  Array.from(cards).filter(c=>c.offsetTop === clickedOffsetTop).includes(cards[activeCardIndex]);

                if (activeCardIndex !== -1 && isSameRow) {
                    // Just update content softly
                    isAnimating = true;
                    // Reset previous active arrow
                    if(activeCardIndex !== -1) cards[activeCardIndex].querySelector('.ospite-toggle-icon').style.transform = 'rotate(0deg)';
                    
                    // Activate new one
                    activeCardIndex = index;
                    card.querySelector('.ospite-toggle-icon').style.transform = 'rotate(180deg)';
                    ...
                    setTimeout(() => {
                        ...
                        const innerHTH = detailsBox.querySelector('.guest-details-inner').offsetHeight;
        ...
        function openBox(card, lastInRow) {
            ...
            // Calc height
            setTimeout(() => {
                const height = detailsBox.querySelector('.guest-details-inner').offsetHeight;
        ...
        function closeBox(callback) {
            ...
            card.querySelector('.ospite-toggle-icon').style.transform = 'rotate(0deg)';
    ...
    elementorFrontend.hooks.addAction('frontend/element_ready/grid_ospiti.default', GridOspitiHandler);
