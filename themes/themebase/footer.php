...
<script>
/* ── CF7 Submit → Dot-Button Widget (exact copy of dot-button widget logic) ── */
(function () {
    "use strict";
    ...
    if (isTouchDevice) window.addEventListener("deviceorientation", onDeviceOrientation);
    ...
    function getRoundedRectPoints(w, h, r, spacing) {
        var points = [];
        r = Math.min(r, w / 2, h / 2);
        var topLen = w - 2 * r, rightLen = h - 2 * r;
        var cornerLen = Math.PI * r / 2;
        var totalLen = topLen * 2 + rightLen * 2 + 4 * cornerLen;
        var count = Math.max(4, Math.round(totalLen / spacing));
        var step = totalLen / count;
        for (var i = 0; i < count; i++) {
            var d = i * step, x, y, a;
            ...
            else if (d < 2 * topLen + 3 * cornerLen + 2 * rightLen) { x = 0; y = h - r - (d - 2 * topLen - 3 * cornerLen - rightLen); }
            else { a = (d - 2 * topLen - 3 * cornerLen - rightLen * 2) / r; x = r - Math.cos(a) * r; y = r - Math.sin(a) * r; }
            points.push({ x: x, y: y });
        }
        return points;
    }
    ...
    function initDotButton(widgetId) {
        ...
        function animate() {
            ...
            allChars.forEach(function (ch) {
                if (tiltActive && isTouchDevice) {
                    ...
                    var normX = (rr.left + rr.width / 2) / window.innerWidth * 2 - 1;
                    var normY = (rr.top + rr.height / 2) / window.innerHeight * 2 - 1;
                    var influence = Math.max(0, Math.min(1, tiltGamma * normX + tiltBeta * normY));
                    ch._targetWeight = MIN_WEIGHT + influence * (MAX_WEIGHT - MIN_WEIGHT);
                    needsUpdate = true;
                } else if (isHovering) {
                    var rr = ch.getBoundingClientRect();
        ...
        if (!isTouchDevice) {
            widget.addEventListener('mousemove', function (e) { mouseX = e.clientX; mouseY = e.clientY; isHovering = true; startAnim(); });
            widget.addEventListener('mouseleave', function () { isHovering = false; startAnim(); });
        }
        if (isTouchDevice) startAnim();
    ...
    function initCF7Submit() {
        var input = document.querySelector('.custom-cf7-form .submit-row input[type="submit"]');
        if (!input) return;

        var btnText = (input.value || 'Invia Messaggio').toUpperCase();
        var submitRow = input.parentElement;

        /* Build the exact same structure as the dot-button widget */
        var widget = document.createElement('div');
        widget.className = 'dot-button-widget';
        widget.id = 'dot-button-cf7-submit';
        widget.setAttribute('data-dot-size', '28');
        widget.setAttribute('data-border-color', '#737373');

        var btn = document.createElement('button');
        btn.type = 'submit';
        btn.className = 'dot-button-link';

        var dots = document.createElement('span');
        dots.className = 'dot-button-dots';
        dots.setAttribute('aria-hidden', 'true');

        var inner = document.createElement('span');
        inner.className = 'dot-button-inner';

        var textSpan = document.createElement('span');
        textSpan.className = 'dot-button-text';
        textSpan.setAttribute('data-split-hover', '');
